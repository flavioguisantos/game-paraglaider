...
O fluxo de interface tambem apresenta um tutorial de primeiro voo por veiculo, salvo localmente apos dispensar ou concluir. O painel de camadas abre sob demanda pelo botao de configuracao, em vez de ocupar a cena continuamente; no mobile, o ranking abre por controle compacto durante a rodada.
...
### Hierarquia de leitura da cena 3D
`src/camera.js` aproxima a camera panoramica de espera ate o enquadramento de voo em uma transicao suave de 2,8 s. `src/scoring.js` destaca o waypoint atual e limita a presenca visual dos proximos pontos a 15 km, enquanto a linha translucida respeita a oclusao do relevo. `src/thermal.js` reduz as ajudas visuais por distancia sem mudar sustentacao ou ciclo das termicas: detalhes completos ate 2,6 km, coluna discreta ate 6,5 km e nuvem/aves ate 12 km. `src/vegetation.js` cria uma clareira de 70 m na origem da rampa, preserva densidade junto ao piloto e afina as instancias distantes; revisa o plantio quando a altitude muda para restaurar a cobertura durante a descida, mantendo os volumes de colisao da mata renderizada. `src/buildings.js` desenha referencias visuais de decolagem e aceita marcador de pouso em `landingZone` com latitude/longitude no catalogo, sem inventar coordenadas para os locais existentes. Hierarquia visual, camera, rota e termicas nao mudam regras de pontuacao ou voo; a clareira da rampa tambem remove colisores de arvores nessa area.
