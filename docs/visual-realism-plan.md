...
- Painel "Camadas" na tela (canto superior direito): liga/desliga cada camada vetorial (rodovias, estradas, ferrovias, rios, lagos, areas urbanas, nomes de cidades) via `terrain.setLayerVisibility`, para diagnostico e preferencia do jogador. Seu cabecalho alterna a lista entre aberta e recolhida, e o inicio efetivo do voo a recolhe automaticamente; o painel de configuracao pre-voo fica centralizado para nao competir com ele. Em telas ate 640 px, o painel fica oculto para preservar espaco de HUD e controles por toque.
...
- Mascara de vegetacao OSM: areas residenciais/comerciais/industriais, edificios, ruas, ferrovias e aerovias Shortbread sao rasterizados em uma grade binaria de 24 m por chunk e combinados com a mascara XCM. A chegada dos dados refaz as instancias e os volumes de colisao das arvores; parques, florestas, pomares, gramados e areas naturais continuam aptos a receber vegetacao.
- Superficie viaria OSM sobre o terreno local: rodovias principais usam asfalto opaco de 26 m e ruas locais usam 12 m, ambas drapejadas acima do relevo e ligadas ao mesmo checkbox. A largura cobre o shader verde XCM no leito da via, enquanto a mascara binaria impede copas e colisoes de arvores no corredor adjacente.
- Cobertura urbana OSM: usos residenciais/comerciais/industriais e equipamentos urbanos recebem solo neutro opaco, separado de parques e classes rurais. Lacunas sem poligono de `landuse` sao preenchidas por uma grade de 96 m somente quando a rede de ruas apresenta densidade e orientacoes cruzadas de quarteirao; a mesma cobertura remove arvores e volumes de colisao.
...
- Ceu, sol e sombra de nuvens: `main.js` passou a usar cor de fallback azul, nevoa mais azulada e uma cupula de shader com gradiente atmosferico azul controlado, alem de um sprite `SunVisual` alinhado a `SUN_DIRECTION`/`DirectionalLight`. As sombras manuais das cumulus de termica ficaram maiores e mais fortes, com textura irregular em `clouds.js`, orientacao pelo azimute do sol e altura baseada em `terrain.getRenderedHeightAt` quando disponivel.
